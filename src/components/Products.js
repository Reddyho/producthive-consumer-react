...
function Products() {
  ...
  //state management using useState() ReactHook   -ush shortcut
  ...
  const [user, setUser] = useState('');
   ...
   /*
    ...
    // handling componentDidMount()
     //rect Hook-Runs only once in the first render (ueh - shortcut)
  useEffect(() => {
      fetchProducts();
  }, []);
  ...
  const fetchProducts = () => { 
    ...
    ProductService.getProducts().then((response) => {
      setProduct(response.data); //setting response to state
    });
   }
